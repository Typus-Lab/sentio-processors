...
tds_authorized_entry
  ...
  .onEventUpdateStrikeEvent((event, ctx) => {
    ctx.eventLogger.emit("UpdateStrike", {
      ...
      strikes: event.data_decoded.vault_config.payoff_configs
        .map((config) => config.strike?.toString())
        .join("   "),
    });
  });
...
typus_dov_single
  ...
  .onEventDeliveryEvent(async (event, ctx) => {
    ...
    ctx.meter
      .Gauge("deliveryPremium")
      .record(bidder_bid_value + bidder_fee + incentive_bid_value + incentive_fee, {
        index: event.data_decoded.index.toString(),
        coin_symbol: b_token,
      });
    ...
    if (ids.length > 0) {
      ...
      for (const object of res) {
        ...
        ctx.meter
          ...
          .record(Number(fields.active_share_supply) / 10 ** token_decimal(deposit_token), {
            index,
        ...
        ctx.meter
          ...
          .record(Number(fields.inactive_share_supply) / 10 ** token_decimal(deposit_token), {
            index,
            coin_symbol: deposit_token,
          });
        ctx.meter
          .Gauge("warmup_share_supply")
          .record(Number(fields.warmup_share_supply) / 10 ** token_decimal(deposit_token), {
            index,
            coin_symbol: deposit_token,
          });
        ctx.meter
          .Gauge("premium_share_supply")
          .record(Number(fields.premium_share_supply) / 10 ** token_decimal(bid_token), {
            index,
            coin_symbol: deposit_token,
          });
      }
